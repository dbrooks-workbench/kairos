export async function onRequestPost(context) {
  ...
  if (sessionId) {
    ...
    if (session) {
      ...
      if (accountId) {
        // Remove a single account; keep the session alive if others remain.
        ...
        // Last account removed — fall through to full session teardown
      } else {
        // Revoke all refresh tokens
        await Promise.all(accounts.map(a =>
          a.refreshToken
            ? fetch(`https://oauth2.googleapis.com/revoke?token=${encodeURIComponent(a.refreshToken)}`, { method: 'POST' }).catch(() => {})
            : Promise.resolve()
        ))
      }
...
function migrateSession(session) {
  ...
  if (session.refreshToken) return [{ id: 'legacy', email: null, ...session }]
  return []
}
