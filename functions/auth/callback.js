export async function onRequestGet(context) {
  ...
  // PKCE entry is JSON (new) or a plain string (pre-multi-account sessions)
  ...
  if (existingSessionId) {
    ...
    if (existing) {
      ...
      // New account is secondary; primary flag is already on the first account
      const idx = accounts.findIndex(a => a.id === accountId)
      if (idx >= 0) {
        accounts[idx] = { ...accounts[idx], ...newAccount }
      } else {
        accounts.push({ ...newAccount, primary: false })
      }
  ...
  if (!sessionData) {
    // First account in a new session is the primary
    sessionId   = crypto.randomUUID()
    sessionData = { accounts: [{ ...newAccount, primary: true }] }
  }
...
// Old sessions become the primary account.
