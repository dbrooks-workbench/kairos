...
- `GoogleCalendarProvider` parses event descriptions for: (1) a single embedded JSON object (first `{` to matching `}`, depth-tracked, silent fail) → `metadata.config`; (2) `@timestamp` comment lines → `metadata.comments`. Structured action comments use `$verb` (not `@verb`) to distinguish from the timestamp `@` prefix — sigil grammar: `@` = when, `$` = action verb, `!` = key reference. Example: `@2025-11-15T09:23:00 $spawned !PAY-TAX tasks/abc123xyz`. The JSON config object may contain `tasks` (linked task IDs) and `spawn` (task prototypes) keys.
