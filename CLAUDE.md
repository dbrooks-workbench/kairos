...
- `GoogleCalendarProvider` should parse event descriptions for: (1) `---tasks---` blocks → `metadata.linked_task_ids`; (2) `---spawn:KEY---` blocks → `metadata.spawn_prototypes`; (3) `@timestamp` comment lines → `metadata.comments` (same convention as task notes, including structured action comments like `@spawned !KEY payload`).
- **Spawn triggers** (future feature): named `---spawn:KEY---` blocks in event descriptions define task prototypes (`~trigger:-30d`, `~due:-5d`, `~title:`, LOE, checklist). Kairos spawns a Google Task when entering the trigger window and records `@timestamp @spawned !KEY tasks/{id}` in the event's comment log. Spawn state is determined by parsing `@spawned !KEY` comments — no separate writeback field. Multiple prototypes per event handled via distinct keys. Do not implement until core calendar + task rendering is complete.