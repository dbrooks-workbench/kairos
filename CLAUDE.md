...
- `GoogleCalendarProvider` parses event descriptions for: (1) a single embedded JSON object (first `{` to matching `}`, depth-tracked, silent fail) → `metadata.config`; (2) `@timestamp` comment lines → `metadata.comments` (same convention as task notes, including structured action comments like `@spawned !KEY payload`). The JSON config object may contain `tasks` (linked task IDs) and `spawn` (task prototypes) keys.
