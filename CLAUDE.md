...
- **Spawn triggers** (future feature): calendar events can carry a `---spawn---` block defining a task prototype (`~trigger:-30d`, `~due:-5d`, checklist, LOE) that Kairos materializes into a Google Task when the trigger window is entered. Kairos writes `~spawned:{taskId}` back to the event to prevent re-spawning. Recurring events need per-occurrence spawn tracking. Do not implement until core calendar + task rendering is complete.